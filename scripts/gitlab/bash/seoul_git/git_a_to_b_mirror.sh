...
while :; do
  ...
  # ssh_url_to_repo 와 last_activity_at 추출 후 필터링
...
if [ -f "$FAILED_LIST_FILE" ]; then
  ...
  #echo "$(date '+%Y-%m-%d %H:%M:%S') [info] Not Found previous failed project list" >> $LOG_FILE
  project_names=()
fi
...
for repo_name in "${project_names[@]}"; do
  ...
  echo "$(date '+%Y-%m-%d %H:%M:%S') [info] Processing $repo_name" >> $LOG_FILE
